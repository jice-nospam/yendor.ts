...
export class TopologyAnalyzer {
    ...
    private hasDoor(map: Map, pos: Core.Position): boolean {
        return Actors.Actor.list.filter((actor: Actors.Actor) => actor.pos.equals(pos) && actor.isA("door")).length > 0;
    }
    ...
    */
    findDungeonExits(entry: Core.Position, exit: Core.Position, noDeadEndExit: boolean = false): void {
