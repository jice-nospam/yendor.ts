...
import {KeyCodeEnum, MouseButtonEnum, EVENT_START_DRAG, EVENT_CANCEL_DRAG, EVENT_END_DRAG} from "./constants";
import {resetAxes, updateMouseButtonAxes, updateAsciiCodeAxes, updateKeyCodeAxes} from "./input_virtual";
...
export function onMouseDown(event: JQueryMouseEventObject) {
    ...
    if ( isMouseButtonDown(MouseButtonEnum.LEFT)) {
        EventManager.publishEvent(EVENT_START_DRAG, {startCellPos: dragCellPos, startPos: dragPos});
    }
...
export function onMouseUp(event: JQueryMouseEventObject) {
    ...
    if ( !mouseStatus.pixelPos.equals(dragPos) ) {
        EventManager.publishEvent(EVENT_END_DRAG, {startCellPos: dragCellPos, startPos: dragPos} );
    } else {
