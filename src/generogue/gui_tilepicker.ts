...
export class TilePicker extends Gui.Widget implements Umbra.EventListener, Actors.TilePicker {
        ...
        if (hasRange || hasRadius) {
            ...
            for (pos.x = 0; pos.x < Map.Map.current.w; ++pos.x) {
                for (pos.y = 0; pos.y < Map.Map.current.h; ++pos.y) {
                    let atRange: boolean = hasRange ? Core.Position.distance(this.origin, pos) <= this.range : true;
                    let inRadius: boolean = this.tileIsValid && hasRadius ? Core.Position.distance(this.tilePos, pos) <= this.radius : false;
    ...
    checkKeyboardInput() {
        if (Actors.getLastPlayerAction() === Actors.PlayerAction.CANCEL) {
            this.hide();
            Umbra.resetInput();
            return;
        }
        let action: Actors.PlayerAction = Actors.getLastPlayerAction();
        ...
        switch (action) {
            case Actors.PlayerAction.CANCEL:
                this.hide();
                Umbra.resetInput();
                break;
            case Actors.PlayerAction.VALIDATE:
                if (this.tileIsValid) {
                    this.hide();
                    Umbra.resetInput();
    ...
    handleMouseClick() {
        if (Umbra.wasMouseButtonReleased(Umbra.MouseButton.LEFT)) {
            if (this.tileIsValid) {
                this.hide();
                this.resolve(this.tilePos);
            }
        } else if (Umbra.wasMouseButtonReleased(Umbra.MouseButton.RIGHT)) {
            this.hide();
        }
    }
